<template>
  <div class="container px-2 mx-auto">
    <h1 class="mb-4 text-lg font-bold">
      Sobre este site
    </h1>
    <p>Este site compila os dados disponibilizados pela prefeitura de Poços de Caldas - MG, sobre Covid-19.</p>
    <p>Todos os dados originais podem ser encontrados aqui: <a href="https://pocosdecaldas.mg.gov.br/" class="text-blue-900 underline">https://pocosdecaldas.mg.gov.br/</a></p>
    <p>A ideia desse site é apenas exibir os dados de forma mais amigável e compilada.</p>
  </div>
</template>

<style scoped>
p {
  @apply mb-2
}
</style>
